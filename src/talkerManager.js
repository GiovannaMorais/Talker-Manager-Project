...
const findTalkers = async (req, res) => {
  const { q } = req.query;
  const talkersLists = await getAllTalkers();
  const verify = talkersLists.filter((talker) =>
    talker.name.toLowerCase().includes(q.toLowerCase()));
  if (!q || q === ' ') return res.status(200).json(talkersLists);
  if (!verify) return res.status(200).json([]); 
